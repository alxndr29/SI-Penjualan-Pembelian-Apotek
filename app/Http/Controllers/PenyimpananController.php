...
class PenyimpananController extends Controller
{
    ...
    public function stokBarangView($month = null)
    {
        ...
        if ($month != null) {
            $stockProduct = DB::table('products as p')
                ->select(
                    'p.nama',
                    db::raw("(SELECT DOX.stock_aktual FROM detail_stock_opname AS DOX INNER JOIN stock_opname AS opn WHERE opn.state = 'Finish' AND dox.product_id = p.id AND opn.bulan = '" . $month . "') AS stok_awal"),
                    db::raw("(SELECT SUM(si.jumlah) FROM stock_in AS si INNER JOIN purchase_order AS po ON si.purchase_order_id = po.id WHERE po.state = 'Lunas' AND si.product_id = p.id AND MONTH(po.transaction_date) = '" . $month . "') AS stok_masuk"),
